...
window.GokoSalvager.createSettingsDialog = function () {
    ...
    return $('\
<div id="usDialog" title="User Settings" ng-app ng-controller="gokoSalvagerUserSettingsController" >\
  Autokick:<br>\
  ...
  Blacklist (kick, censor, never automatch):\
  <table>\
    <tbody>\
  ...
  Automatch Blacklist (do not kick or censor):\
  <table>\
    <tbody>\
      <tr ng-repeat="pname in us.automatch_blacklist">\
        <td><label style="color:red" ng-click="rem_from_automatch_blacklist(pname)">X</label> </td>\
        <td>{{pname}}</td>\
        <td></td>\
        <td></td>\
      </tr>\
      <tr>\
        <td></td>\
        <td><input type="text" ng-model="temp_ambl" /></td>\
        <td><button ng-click="add_to_automatch_blacklist(temp_ambl)" >Add</button></td>\
      <tr>\
    </tbody>\
  </table>\
</div>');
